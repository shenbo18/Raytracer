...
void PhotonMappingRenderer::TracePhoton(PhotonKdtree& photonMap, Ray* photonRay, glm::vec3 lightIntensity, std::vector<char>& path, float currentIOR, int remainingBounces)
{
	...
	//if (remainingBounces == 998) {
	//	std::cout << "second bounce" << std::endl; };


    assert(photonRay);
    IntersectionState state(0, 0);
    state.currentIOR = currentIOR;
	bool intersected = storedScene->Trace(photonRay, &state);
	...
	if (path.size() != 1) { 
		...
		Photon myPhoton;
		Ray toLightRay;
		toLightRay.SetRayDirection(-1.0f*photonRay->GetRayDirection());
		toLightRay.SetRayPosition(intersectionPoint);
	...
	float Pr = glm::compMax(diffuseReflection);
	//std::cout << "Pr" << Pr << std::endl;

	std::random_device rd;
	std::default_random_engine generator(rd());
	std::uniform_real_distribution<float> distribution(0.0f, 1.0f);
	float rndNum = distribution(generator);
	//std::cout << rndNum << std::endl;
	if (rndNum > Pr) { return; }; //Absorption
	...
	float y = r*sin(theta);
	float z = sqrt(1 - u1);
	glm::vec3 reflectionRayDirection = glm::normalize(glm::vec3(x, y, z));

	// Hemisphere transformation
	glm::vec3 n = state.ComputeNormal();
	glm::vec3 t = glm::normalize(glm::cross(n, glm::vec3(1.0, 0.0, 0.)));
	if (1-abs(glm::dot(n, glm::vec3(1.0, 0., 0.)))< 0.3)
	{
		t = glm::normalize( glm::cross(n, glm::vec3(0.0, 1.0, 0.)));
	}
	...
	glm::vec3 newPhotonDirection = glm::normalize(trans *  reflectionRayDirection);
	Ray newPhotonRay;
	newPhotonRay.SetRayDirection(glm::normalize(newPhotonDirection));
	newPhotonRay.SetRayPosition(intersectionPoint+0.001f*n);
	path.push_back('s');
	PhotonMappingRenderer::TracePhoton(photonMap, &newPhotonRay, lightIntensity, path, currentIOR, remainingBounces - 1);
